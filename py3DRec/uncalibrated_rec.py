...
class UncalibratedReconstruction: 
    ...
    def bundle_adjustment(self,feat_2D,P,feat3D): 
        ...
        X=np.append(X,np.reshape(feat3D[:,0:3],number_of_features*3))
        ...
        feat3D[:,0:3]=np.reshape(X[self._sequence_length*11:self._sequence_length*11+number_of_features*3],(number_of_features,3))
...
def main(show):
    ...
    images = reconstruction.images_from_video(vid_path)
    ...
    sequence=ImageSequence(features,image_samples);
